...
def main():
    ...
    # csv_file_path = "/Users/boohee/Workspace/llms/learn-agents/learn-agents-v0/dataset/支付宝-1118.csv"
    #
    # # 读取图片URL
    # image_urls = read_image_urls_from_csv(csv_file_path)
    #
    # # 只处理前3张图片作为示例
    # sample_image_urls = image_urls[:3]
    #
    # # 批量处理图片，控制并发数为3
    # results = asyncio.run(ImageIdentifyAgent.batch_run(sample_image_urls, max_concurrent=3))
    #
    # # 打印结果
    # print("\n从CSV文件读取的图片处理结果:")
    # for result in results:
    #     if result["status"] == "success":
    #         print(f"✓ 图片识别成功: {result['result']}")
    #     else:
