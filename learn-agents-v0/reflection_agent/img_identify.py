...
class ImageIdentifyAgent:
    ...
    @classmethod
    async def run(cls, image_url: str, max_count: int = 3) -> dict:
        """
        类方法：识别单张图片
        ...
        Returns:
            dict: 识别结果
        """
        return await cls(image_url, max_count).image_identify()

    @classmethod
    async def run_classmethod(cls, image_url: str, max_count: int = 3) -> dict:
        """
        类方法：识别单张图片
        ...
        Returns:
            dict: 识别结果
        """
        return await cls(image_url, max_count).run()

    @classmethod
    async def batch_run(cls, image_urls: List[str], max_concurrent: int = 5, max_count: int = 3) -> List[Dict[str, Any]]:
        """
        批量处理图片，支持控制并发数
        ...
        Args:
            ...
            max_count (int): 每张图片的最大重试次数，默认为3
            
        Returns:
            List[Dict[str, Any]]: 批量处理结果列表
        """
        # 创建信号量控制并发数
        semaphore = asyncio.Semaphore(max_concurrent)
        
        async def process_single_image(image_url: str) -> Dict[str, Any]:
            """处理单张图片"""
            async with semaphore:  # 控制并发数
                try:
                    agent = cls(image_url, max_count)
                    result = await agent.image_identify()
                    return {
                        "image_url": image_url,
                        "result": result["generate_ret"],
